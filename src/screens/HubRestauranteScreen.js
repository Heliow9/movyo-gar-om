...
const emptyProduto = () => ({ nome: "", descricao: "", precoBase: "", imagem: "", categoria: "", tipoItem: "comum", maxSabores: 1, calculoPrecoPor: "maior", pizzaMultisabor: false, sabores: [], bordas: [], adicionais: [], complementos: [], extras: {}, receita: "", destaque: false, ativoVitrine: true, imprimir: true });
...
export default function HubRestauranteScreen({ onLogout }) {
  ...
  useEffect(() => {
    ...
    const notificarCaixaFechado = (payload = {}) => {
    ...
    eventosCaixaFechado.forEach((ev) => socket?.on?.(ev, notificarCaixaFechado));
    ...
    return () => {
      ...
      eventosCaixaFechado.forEach((ev) => current?.off?.(ev, notificarCaixaFechado));
  ...
  const iniciarEdicaoProduto = (p) => {
    ...
    setProdutoForm({ ...emptyProduto(), ...p, categoria: getId(p.categoria) || p.categoria || "", tipoItem: p.tipoItem || p.tipo || "comum", maxSabores: Number(p.maxSabores || 1), calculoPrecoPor: p.calculoPrecoPor || "maior", pizzaMultisabor: !!p.pizzaMultisabor || Number(p.maxSabores || 1) > 1, precoBase: String(p.precoBase ?? p.preco ?? ""), sabores: p.sabores || [], bordas: p.bordas || [], adicionais: p.adicionais || [], complementos: p.complementos || [], extras: p.extras || {}, ativoVitrine: p.ativoVitrine !== false, imprimir: p.imprimir !== false && p.imprimeNaCozinha !== false });
    setTab("produtos");
  };
  ...
  const salvarProduto = async () => {
    ...
    await runAction(produtoEditandoId ? "Salvando produto..." : "Criando produto...", async () => {
      const tipoItem = produtoForm.tipoItem === "pizza" ? "pizza" : "comum";
      const maxSabores = tipoItem === "pizza" ? Math.max(1, Math.min(12, Number(produtoForm.maxSabores || 1))) : 1;
      const payload = { ...produtoForm, restaurante: restauranteId, nome: produtoForm.nome.trim(), imagem: produtoForm.imagem || MOCK_IMAGE, precoBase: preco, preco, tipoItem, tipo: tipoItem, maxSabores, pizzaMultisabor: tipoItem === "pizza" && maxSabores > 1, calculoPrecoPor: tipoItem === "pizza" && maxSabores > 1 ? produtoForm.calculoPrecoPor || "maior" : "maior", ativo: true, disponivel: true, ativoVitrine: produtoForm.ativoVitrine !== false, imprimir: !!produtoForm.imprimir, imprimeNaCozinha: !!produtoForm.imprimir, categoria: produtoForm.categoria };
  ...
  const registrarMovimentoCaixa = async () => {
    if (caixa?.status !== "aberto") return Alert.alert("Caixa fechado", "Abra o caixa antes de registrar sangria ou suprimento.");
    const valor = Number(onlyNumber(movimentoCaixaForm.valor));
    if (!Number.isFinite(valor) || valor <= 0) return Alert.alert("Valor invalido", "Informe um valor maior que zero.");
    await runAction("Registrando movimento...", async () => {
      await api.post(`/api/caixa/${restauranteId}/movimento`, {
        tipo: movimentoCaixaForm.tipo,
        valor,
        descricao: movimentoCaixaForm.descricao,
      });
      setMovimentoCaixaForm({ tipo: "sangria", valor: "", descricao: "" });
      await load({ silent: true });
    }).catch((e) => Alert.alert("Erro", e?.response?.data?.message || e?.response?.data?.mensagem || e.message));
  };
  ...
  const atualizarStatusPedidoHub = async (pedido, novoStatus) => {
    if (novoStatus === "cancelado") {
      return Alert.alert("Cancelar pedido", "Cancelar este pedido e executar estorno quando aplicavel?", [
        { text: "Voltar", style: "cancel" },
        { text: "Cancelar pedido", style: "destructive", onPress: () => atualizarStatusPedidoHub(pedido, "cancelado_confirmado") },
      ]);
    }
    const pedidoId = getId(pedido);
    if (!pedidoId) return;
    const statusFinal = novoStatus === "cancelado_confirmado" ? "cancelado" : novoStatus;
    const labels = {
          ...
          {tab === "caixa" && <>
            <Card title="Abertura e fechamento" icon="cash-outline" subtitle="O fechamento exige o PIN do operador que abriu o caixa quando houver PIN cadastrado.">
              {caixa?.status === "aberto" ? <>
                ...
                <Text style={styles.label}>Sangria / Suprimento</Text>
                <View style={styles.chipRow}>
                  <OptionChip label="Sangria" icon="arrow-up-circle-outline" active={movimentoCaixaForm.tipo === "sangria"} onPress={() => setMovimentoCaixaForm({ ...movimentoCaixaForm, tipo: "sangria" })} />
                  <OptionChip label="Suprimento" icon="arrow-down-circle-outline" active={movimentoCaixaForm.tipo === "suprimento"} onPress={() => setMovimentoCaixaForm({ ...movimentoCaixaForm, tipo: "suprimento" })} />
                </View>
                <Field label="Valor do movimento" value={movimentoCaixaForm.valor} onChangeText={(v) => setMovimentoCaixaForm({ ...movimentoCaixaForm, valor: v })} keyboardType="decimal-pad" />
                <Field label="Motivo" value={movimentoCaixaForm.descricao} onChangeText={(v) => setMovimentoCaixaForm({ ...movimentoCaixaForm, descricao: v })} multiline />
                <Button title="Registrar movimento" icon="swap-vertical-outline" onPress={registrarMovimentoCaixa} disabled={!!actionLabel} />
  ...
  return (
    <Card title={title} icon={aReceber ? "notifications-outline" : "receipt-outline"} subtitle={subtitle || `${list.length} pedido(s) exibido(s)`}>
      {list.length ? list.map((pedido) => {
        ...
        const nextActions = [];
        if (["", "novo", "pendente", "recebido", "pago", "aguardando", "aguardando_confirmacao"].includes(status)) {
          nextActions.push(["Aceitar", "em_producao", "checkmark-circle-outline"]);
...
function ProdutosView(props) {
  ...
  return <>
    ...
    {(produtoForm.tipoItem === "pizza" || categoriaSelecionada?.permiteSabores) ? <GrupoProduto title="Sabores" groupKey="sabores" items={produtoForm.sabores} temp={tempInputs.sabores} setTemp={(obj) => setTempInputs({ ...tempInputs, sabores: obj })} onAdd={() => adicionarItemProduto("sabores")} onRemove={(i) => removerItemProduto("sabores", i)} /> : null}
    {(produtoForm.tipoItem === "pizza" || categoriaSelecionada?.permiteBordas) ? <GrupoProduto title="Bordas" groupKey="bordas" items={produtoForm.bordas} temp={tempInputs.bordas} setTemp={(obj) => setTempInputs({ ...tempInputs, bordas: obj })} onAdd={() => adicionarItemProduto("bordas")} onRemove={(i) => removerItemProduto("bordas", i)} /> : null}
    {(produtoForm.tipoItem === "pizza" || categoriaSelecionada?.permiteAdicionais) ? <GrupoProduto title="Adicionais" groupKey="adicionais" items={produtoForm.adicionais} temp={tempInputs.adicionais} setTemp={(obj) => setTempInputs({ ...tempInputs, adicionais: obj })} onAdd={() => adicionarItemProduto("adicionais")} onRemove={(i) => removerItemProduto("adicionais", i)} /> : null}
